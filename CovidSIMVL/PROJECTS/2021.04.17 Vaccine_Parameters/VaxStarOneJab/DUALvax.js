...
//        return(b+pb);
 ...
 change enter new value between 5.2 and 13.2 or cancel (applies to REDs)", VLinfEnd);
  ...
  function showVax(){
    ...
    U[vU].vaxPercent = Math.round(txt.substring(y+1));    //refers to percentage of population vaccinated
    ...
    function VLtransfer(i, j) {
        ...
        */
        ...
        if (VaxProb < prProtect) {
          console_log("Vax prevented "+Vsmall+" ageGp"+P[Vsmall].ageGp+" by "+Vbig+" ageGp"+P[Vbig].ageGp+" gen"+gen+" Protection Level "+prProtect+" Prob "+VaxProb+" Sector"+vSector);
          return
        };

        TransProb = Math.random();
        if (TransProb > infProb) {
          console_log("No transmit "+Vbig+ " ageGp"+P[Vbig].ageGp+" on "+Vsmall+" ageGp"+P[Vsmall].ageGp+" gen"+gen+"Transmit Level "+infProb+" Prob "+TransProb);
          return
        };
