...
//        return(b+pb);
    ...
    function parseL(lineStr) {
		...
		let trAgeGp = eval(lineS[8]);
		let trFam = lineS[9];
		if (ID==pID) {return true}
		else {
			P[ID].ageGp = -1;
			if (trAgeGp!="" && trAgeGp!==undefined){P[ID].ageGp=trAgeGp};
			P[ID].famKey = -1;
 ...
 change enter new value between 5.2 and 13.2 or cancel (applies to REDs)", VLinfEnd);
  ...
  function showVax(){
    ...
    let y = txt.indexOf(",",0);
    if (y==-1) {return};
    U[vU].vaxAgeGp = eval(txt.substring(0,y));
    U[vU].vaxGroup = eval(txt.substring(y+1));
    U[vU].vaxMode = 2;    // for now, always 2
    console.log("AgeGp ="+U[vU].vaxAgeGp+" vaccinated="+U[vU].vaxGroup+"%");
...
function implementVax(ages,vax){
    ...
    for (let i=0;i<aCt; i++){
      let k = U[vU].person[i];
      if (P[k].ageGP == ages) console.log("equal") else console.log("not equal");
      if (P[k].state != "green" || P[k].ageGP != ages){
         continue}
      else {
         susList[susvxCt] = k;
         susvxCt++;
      ...
      console.log("ageGp"+ages+": "+susList);
  ...
  /* now take % away
    ...
    */
      ...
      console.log("AgeGp"+ages+" vaccinated "+vaxed);
