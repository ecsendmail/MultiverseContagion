	...
	slider.oninput = function () {
		...
		clockTimer = setInterval(TimesUp, MOTION / FPS);
            ...
            */
            ...
            function showMV() {
                ...
                if (M.UCt > 0) { document.getElementById("MVstats0").style.display = "block" };
                if (M.UCt > 1) { document.getElementById("MVstats1").style.display = "block" };
                if (M.UCt > 2) { document.getElementById("MVstats2").style.display = "block" };
                if (M.UCt > 3) { document.getElementById("MVstats3").style.display = "block" };
                if (M.UCt > 4) { document.getElementById("MVstats4").style.display = "block" };
                if (M.UCt > 5) { document.getElementById("MVstats5").style.display = "block" };
                if (M.UCt > 6) { document.getElementById("MVstats6").style.display = "block" };
                if (M.UCt > 7) { document.getElementById("MVstats7").style.display = "block" };
                if (M.UCt > 8) { document.getElementById("MVstats8").style.display = "block" };
            ...
            */
            ...
            function parseL(lineStr) {
                ...
                let ID = eval(lineS[0]);          // eval() takes variable name and returns value
                transfer.pID = ID;
            ...
            var ageRiskT = [];
            ageRiskT[0] = { low: 0, high: 9, risk: 0.33 };
            ageRiskT[1] = { low: 10, high: 19, risk: 0.44 };
            ageRiskT[2] = { low: 20, high: 29, risk: 0.90 };
            ageRiskT[3] = { low: 30, high: 39, risk: 1.10 };
            ageRiskT[4] = { low: 40, high: 49, risk: 1.08 };
            ageRiskT[5] = { low: 50, high: 59, risk: 1.15 };
            ageRiskT[6] = { low: 60, high: 69, risk: 0.96 };
            ageRiskT[7] = { low: 70, high: 79, risk: 1.05 };
            ageRiskT[8] = { low: 80, high: 89, risk: 1.49 };
            ageRiskT[9] = { low: 90, high: 99, risk: 2.33 };
      ...
      //          document.getElementById("dCycl").innerHTML = cycleCount;
            ...
            function VLtransfer(i, j) {
                ...
                if (ipostInfect > VLincD) { P[i].touchCt++ };
                if (ipostInfect > VLincD && P[j].state == "green") { P[i].susCt++ };
                if (jpostInfect > VLincD) { P[j].touchCt++ };
                if (jpostInfect > VLincD && P[i].state == "green") { P[j].susCt++ };

                let qVir = 0;
                if (Vbig == i) {
                    let jinfD = Math.floor(jpostInfect);
                    if (jinfD < 21) {var jinfVir = tViral[jinfD]}
                        else jinfVir = 1;
                    ...
                    if (iinfD < 21) {var iinfVir = tViral[iinfD]}
                      else iinfVir = 1;
          ...
          function TimesUp() {
            ...
            function tabulate() {
                ...
                for (i = 0; i < M.UCt; i++) {
                    ...
                    let len = 0;
                    Q.endGreen.push({ y: Q.logGreen[gen], x: gen / 24 });
                    Q.endYellow.push({ y: Q.logYellow[gen], x: gen / 24 });
                    Q.endBlue.push({ y: Q.logBlue[gen], x: gen / 24 });
                    Q.endRed.push({ y: Q.logRed[gen], x: gen / 24 });
                    Q.endOrange.push({ y: Q.logOrange[gen], x: gen / 24 });
                    Q.endCases.push({ y: Q.logCases[gen], x: gen / 24 });
                    if (gen > 1) {
                        let del = Q.logCases[gen] - Q.logCases[gen - 1];
                        Q.endRedDelta.push({ y: del, x: gen / 24 });
                    } else
                        {Q.endRedDelta.push({ y: 0, x: gen / 24 })};
            ...
            function Mtabulate() {
                ...
                M.endGreen.push({ y: M.logGreen[gen], x: gen / 24 });
                M.endYellow.push({ y: M.logYellow[gen], x: gen / 24 });
                M.endBlue.push({ y: M.logBlue[gen], x: gen / 24 });
                M.endRed.push({ y: M.logRed[gen], x: gen / 24 });
                M.endOrange.push({ y: M.logOrange[gen], x: gen / 24 });
                M.endCases.push({ y: M.logCases[gen], x: gen / 24 });

                if (gen > 1) {
                    let del = M.logCases[gen] - M.logCases[gen - 1];
                    M.endRedDelta.push({ y: del, x: gen / 24 });
                } else
                    M.endRedDelta.push({ y: 0, x: gen / 24 });
            ...
            function drawNArena() {
                ...
                for (let i = 0; i < 9; i++) {
                    drawNC(i, N[i].x, N[i].y, 20, "midnightblue");
            ...
            function drawNC(i, x, y, rad, color) {       // draw a single node
                gctx.beginPath();
