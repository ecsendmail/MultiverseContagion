...
try {
   ...
   var ri, rj;
    ...
    function drawC(x, y, rad, color) {
        ...
        let xTopLeft, yTopLeft, rWidth, rHeight;
        let xBotRight, yBotRight;

        xTopLeft = Math.round(x-rad);
        yTopLeft = Math.round(y-rad);
        rWidth = Math.round(2*rad);
        rHeight = Math.round(2*rad);

        if (xTopLeft<0){xTopLeft=0};
        if (x+rad>799){
          rWidth = Math.round(rad + 799 - x);
        }
        if (yTopLeft<0){yTopLeft=0};
        if (y+rad>599){
          rHeight = Math.round(rad + 599 - y);
        }


        xBotRight = xTopLeft + rWidth;    // x,y of right bottom of rectangle
        yBotRight = yTopLeft + rHeight;

        ctx.beginPath();
        ctx.strokeStyle = "#0000FF";
        ctx.lineWidth = 1;
        ctx.rect(xTopLeft, yTopLeft, rWidth, rHeight);
        ctx.stroke();
...
//  DRAW GRID
        ...
        arenaPerCent = (arenaCt/480000 * 100).toFixed(2);

        var xSectTL, xSectBR, ySectTL, ySectBR;

        xSectTL = Math.floor(xTopLeft/50);
        ySectTL = Math.floor(yTopLeft/50);
        xSectBR = Math.floor(xBotRight/50);
        ySectBR = Math.floor(yBotRight/50);

        for (si=xSectTL;si<xSectBR+1;si++){
            for (sj=ySectTL;sj<ySectBR+1;sj++){
              sectors[si][sj] = sectors[si][sj]+1;
            }
        }
        ...
        for (si=0;si<16;si++){
          for (sj=0;sj<12;sj++){
            if (sectors[si][sj]>0) {sectorCt++}
          }
        }

        sectorPerCent = (sectorCt/192*100).toFixed(2);
        var mF;
        mF = document.getElementById("dMingl").innerHTML;

       console.log("gen="+gen+" mF="+mF+" x,y="+x.toFixed(2)+","+y.toFixed(2)+" HzR="+rad.toFixed(2)+" arena%="+arenaPerCent+" sectorTL-BR="+xSectTL+","+ySectTL+" "+xSectBR+","+ySectBR+" sectorCt+% "+sectorCt+","+sectorPerCent);

// now we do cluster studies - each position has a sequence number and we will track by x,y and sequence
// number as well as the distance between the current and previous positions
// then we analyze the distances as sequences in which breaks start new clusters...
// breaks will be determined by the difference between current distance and the running average and SD
//could export and do it in R?


        positions[posCt] = x.toFixed(2)+","+y.toFixed(2)+" ";
        posX[posCt] = x;
        posY[posCt] = y;
        if (posCt>0){
          posdX[posCt] = Math.abs(posX[posCt]-posX[posCt-1]);
          posdY[posCt] = Math.abs(posY[posCt]-posY[posCt-1]);
          dXdY[posCt] = (posdX[posCt] + posdY[posCt]).toFixed(0);
        }

        posCt++;

        console.log("posCt"+posCt+" "+positions);
        console.log("dXdY = "+dXdY);
...
//        }
 ...
 change enter new value or cancel (applies to REDs)", metDiagMax);
    ...
    function showMingle(){
      ...
      let mn = eval(txt);
      U[vU].minglf = mn;     // so user can keep the same scale of 1 to 10 for U and Persons
    }
