...
try {
    ...
    */
    ...
    M.UCt = 9;
    console.log(Date());
    console.log("CovidSIMVL Cloverdale U0+U8");
    //console.log("Mode 1 >13 and <42 is 75%");
    ...
    (function() {
        let i;
        oneTime = 0;     // for use by the first routine that needs a one-time switch
        for (i = 0; i < M.UCt; i++) {
...
//        return(b+pb);
    ...
    function changeState(ID) {
        ...
        if (postInfect > 0) {
            if (postInfect < G.tIncubate) {
                if (G.ViralLoad > 0 && G.state != "yellow")
                  {newState(ID, "yellow")};
                return
            }
            if (postInfect < G.tOnset) {
                if (G.ViralLoad > G.qInfective && G.state !="blue")
                  {newState(ID, "blue")};
                return
            }
            if (postInfect < G.tInert) {
                if (G.ViralLoad > G.qInfective && G.state != "red")
                  {newState(ID, "red");
                   console.log(ID+" TURNED RED at gen "+gen+" TOTAL "+M.RedCt);
                  };
                return
            }
            newState(ID, "orange");
 ...
 change enter new value between 5.2 and 13.2 or cancel (applies to REDs)", VLinfEnd);
  ...
  function showMingle(){
         ...
         console.log("new Mingle Factor = "+txt+" in U"+vU);
    		let mn = eval(txt);
    		U[vU].minglf = mn;     // now this is consistent with U and person scales
    	}

  function showVax(){
    let txt = prompt("Enter immunization mode and %  as m,n eg 1,20\nMode 1 - starts day 14 at 75% then drops to zero on day 42\nMode 2 - goes to 95% at day 28");
    document.getElementById("vaxnow").innerHTML = txt;
    console.log("Mode "+txt+" modified");
    let y = txt.indexOf(",",0);
    if (y==-1) {return};
    U[vU].vaxMode = eval(txt.substring(0,y));
    U[vU].vaxGroup = eval(txt.substring(y+1));
    alert("Mode ="+U[vU].vaxMode+" Group="+U[vU].vaxGroup+"%");
...
function implementVax(vax){
    ...
    for (let i=0;i<aCt; i++){
      let k = U[vU].person[i];
      if ((P[k].state != "green") || (P[k].VaxType>0)) {
         continue}
      else {
         susList[susvxCt] = k;
         susvxCt++;
  ...
  function Uinflate(){
    let txt = prompt("Enter the inflation (or deflation) factor for THIS universe eg 1.5 or 0.8");
    let infFactor = eval(txt);
    document.getElementById("Uinf").innerHTML = infFactor;
    if (infFactor == null || infFactor == "" || infFactor == 0) return;
    console.log("inflate universe index "+txt+" in U"+vU);
    U[vU].inflate = infFactor;
    let uCt = U[vU].Population;
    for (let i=0;i<uCt; i++){
      let k = U[vU].person[i];
      P[k].currSize = P[k].currSize * U[vU].inflate;
    }
  }
...
//        }
    ...
    function VLtransfer(i, j) {
      ...
      if (P[Vsmall].ViralLoad!=0 || P[Vsmall].vaxType==0) { }
      else {
      ...
      if (jpostInfect > VLincD && P[i].state == "green") {
        ...
        let qVir = 0;
        let Inf = M.YellowCt+M.BlueCt+M.RedCt+M.OrangeCt;
        if (Vbig == i) {
          ...
          if (P[j].tInfect == 0) {
			    ...
			    if (P[j].state=="green") {
               console.log(Inf+" I j:famKey "+j+":"+P[j].famKey+" infected by "+P[i].state+" i:famKey "+i+":"+P[i].famKey+" at gen "+gen+" in Univ"+wU+" prob="+VaxProb.toFixed(3));
			    }
            ...
            if (P[i].tInfect == 0) { // once for infection
			    ...
			    if (P[i].state=="green"){
				        console.log(Inf+" I i:famKey "+i+":"+P[i].famKey+" infected by "+P[j].state+" j:famKey "+j+":"+P[j].famKey+" at gen "+gen+" in U"+wU+" prob="+VaxProb.toFixed(3));
			    }
        }
    }
    // &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
...
//        }
  ...
  function loadthisHour(){

      let cDcHmov = true;
      let zDcHmov = true;

      // tricky routine here.....use Day 0 as the default SCHEDULE
      // but if on this day cD there is an entry fot this person we use this
      // OTHERWISE we use the entry for this person from Day 0

      // so we want to execute D[0].H[cH] but first do we have any entry for cD.cH?

      // we code 4 ROUTINES
      // ONLY DAY 0 cH current hour default movements
      // only cD cH movements
      // both present same hour - do day 0 cH suppressing cDcH movements
      // finally do cDcH moves

      if (D[cD] === undefined || D[cD].H[cH] === undefined || D[cD].H[cH] == "") {
          cDcHmov = false;
      }

      if (D[0].H[cH] === undefined  || D[0].H[cH] == "") {
          zDcHmov = false;
      }

      if (!cDcHmov && !zDcHmov) {return};    // no movementt this hour

      // now we have the problem that we may have both
      // but if xD == 0 we are sure we have no movements this HOUR
      // if XD at this point is cD we do have at least an entry this hour for cD
      // and we may have an entry for 0 too....


  // now let's do the case that there is no cDcHmov

    if (cDcHmov && zDcHmov && cD>0) {
        testDuplicate();
    } else {
      if (cDcHmov) {xD = cD}
        else {xD = 0}
  ...
  function testDuplicate(){
    let alen = D[0].H[cH].length;
    let win = 0;

    for (let cS=0; cS<alen; cS++){
      let xID = D[0].H[cH][cS].cID;
      let blen = D[cD].H[cH].length;
      for (let bS=0; bS<blen; bS++){
          let bID = D[cD].H[cH][bS].cID;
          if (xID == bID){
            win = cD;
            break;
          }
      ...
      if (win==0) {
          wU = D[0].H[cH][cS].u;
          slot = D[0].H[cH][cS].cS; // the person's slot number
          action = D[0].H[cH][cS].cDir; // the direction
          pID = D[0].H[cH][cS].cID; // the person
          if (action == "A") injectXY(P[pID], pID, slot);
          if (action == "D") expel(P[pID], pID, slot);
      }
      win = 0;
      ...
      // now we have done any default actions from day 0 and suppressed
      // those actions in the same hour which are from current day cD
      // now we do the cD actions

    let blen = D[cD].H[cH].length;
    for (let cS=0; cS<blen; cS++){
        wU = D[cD].H[cH][cS].u;
        slot = D[cD].H[cH][cS].cS; // the person's slot number
        action = D[cD].H[cH][cS].cDir; // the direction
        pID = D[cD].H[cH][cS].cID; // the person
        if (action == "A") injectXY(P[pID], pID, slot);
        if (action == "D") expel(P[pID], pID, slot);
    }
  }
...
*/
    ...
    function reSizeAll() {
        // resize works on P, not from U, so the characteristics
        // need referral to the person's current U as wel
        ...
        for (i = 0; i < M.PCt; i++) {
            ...
            let adjBase = Q.baseSize;
            let R = Q.u;      // the person's current universe for Inflation
            if (R > -1) {
              if (U[R].inflate != 0){
                  adjBase = adjBase * U[R].inflate;
              }
            }
            if (newSize < adjBase) {
                newSize = adjBase;
            } else {
                if (newSize > (adjBase * 3)) {
                    newSize = adjBase * 1.5;
                }
            }
    ...
    function injectXY(G, g, stop) {
        ...
        if (U[wU].inflate !=0) {
          G.currSize = G.currSize * U[wU].inflate;
//          console.log("pID "+G.pID+" inflated to size "+G.currSize+" in "+wU);
        }
    ...
    function expel(G, g, slot) {
        ...
        if (T[g].S[slot].ETD != -1) { // staying
            ...
            if (U[wU].inflate !=0) {
              G.currSize = G.currSize / U[wU].inflate;
//              console.log("pID "+G.pID+" deflated to size "+G.currSize+" in "+wU);
    ...
    function showUstat(Q, q) {
        if (VIEW == "local" && q == vU) {
            ...
            document.getElementById("Uinf").innerHTML = Q.inflate;
