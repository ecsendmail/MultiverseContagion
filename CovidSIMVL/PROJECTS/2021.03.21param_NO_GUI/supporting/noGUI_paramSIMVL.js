...
// ****************************************************************************
console_log("paramSIMVL 2021.03.21 noGUI parameter CovidSIMVL");
console_log(Date());
 ...
 change enter new value between 5.2 and 13.2 or cancel (applies to REDs)", VLinfEnd);
  ...
  function showPradius() {
    ...
    function changeHzR(rTxt){
          ...
          GUI("dPrad",rTxt);
          let HzR = Number(rTxt);
          console_log("New Hazard Radius = "+HzR);
          if (HzR === null || HzR == "" || HzR == VLradius) return;
          VLradius = HzR;
          let pCt = 0;
          for (pCt = 0; pCt < M.PCt; pCt++) {
  ...
  function changeMF(newMF){
        GUI("dMingl",newMF);
        console_log("new Mingle Factor = "+newMF);
        let mn = Number(newMF);
        U[vU].minglf = mn;
  }
...
//          P[j].ViralLoad = 0;
    ...
    function  TimesUp() {
        gen++;
        if (gen % 10 == 0) {console_log("another 10 gens....")};
        let i;
...
/**********************************************************************************/
    ...
    function HALT(){
          wrapUp();
          if (HALTgen >0){
              alertX("HALTgen parameter reached at gen"+gen);
              MODE="manual";
              clearInterval(clockTimer);
