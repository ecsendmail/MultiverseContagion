...
// ****************************************************************************
console_log("paramSIMVL 2021.04.17 DUALUse Vax parameters CovidSIMVL");
console_log(Date());
    ...
    for (i=0;i<lineNo;i++){
        ...
        switch(parx){
          ...
          case "":
              break;
          case "VAX":
              let parVal = lines[i][2];
              let parU   = lines[i][3];
              changeVax(parN, parVal, parU);
              break;
          default:
              break;
        }
...
//      document.getElementById("pane1").innerHTML = UN[0].fontcolor("white");
    ...
    */
    ...
    function initPerson(P, i) {
        ...
        P.vaxType = -1;    // 1 - follows schedule above;  2 - at 28d becomes 95%
        P.vax_Teff1 = 0;
        P.vax_LevelP1 = 0;
        P.vax_Tdec = 0;
        P.vax_Tres = 0;
...
//        return(b+pb);
 ...
 change enter new value between 5.2 and 13.2 or cancel (applies to REDs)", VLinfEnd);
  ...
  function showVax(){
    if (U[vU].vaxMode == -1) {
        alertX("This Universe has no vaccine model - cannot vaccinate population \nCheck Parameter File");
        return
    }
    let txt = prompt("Enter AgeGroup and %  as m,n eg 1,20\n0 = <10; 1=10-19; 2=20-29 etc....9=90+");

    GUI("vaxnow",txt);
    console_log("Age Group "+txt+"% 1st shot");
    let y = txt.indexOf(",",0);
    if (y==-1) {return};
    U[vU].vaxAgeGp = Number(txt.substring(0,y));
    U[vU].vaxPercent = Number(txt.substring(y+1));    //refers to percentage of population vaccinated

    console_log("Gen"+gen+" AgeGp ="+U[vU].vaxAgeGp+" To be vaccinated="+U[vU].vaxPercent+"%");
    let vax = U[vU].vaxPercent;
...
function implementVax(ages,vax){
    ...
    console_log("ageGp"+ages+" count: "+susvxCt+" List: "+susList);
        ...
        let target = vax;     //absolute number to be vaccinated
...
//          P[j].ViralLoad = 0;
          ...
          let X = P[j];
          X.vaxType = 1;
          X.vax_Teff1   = W.vax_Teff1;
          X.vax_LevelP1 = W.vax_LevelP1;
          X.vax_Tdec    = W.vax_Tdec;
          X.vax_Tres    = W.vax_Tres;
          X.vax_LevelR  = W.vax_LevelR;
  ...
  function changeVax(vParName, vParVal, vParU){
      let pU = vParU;
      U[pU].vaxMode = 1;
      switch(vParName){
          case "T.eff1":
              U[pU].vax_Teff1 = Number(vParVal);
              break;
          case "Level.P1":
              U[pU].vax_LevelP1 = Number(vParVal);
              break;
          case "T.dec":
              U[pU].vax_Tdec = Number(vParVal);
              break;
          case "T.res":
              U[pU].vax_Tres = Number(vParVal);
              break;
          case "Level.R":
              U[pU].vax_LevelR = Number(vParVal);
              break;
      }
  }
...
//                if (touchFlag){
//                    if (G.state != "Orange") {G.touchCt++};
//                    if (P[k[j]] != "Orange") {P[k[j]].touchCt++};
//                }
                if (touchFlag && (wU != 8)) {
                      VLtransfer(g, k[j]);
                      return;
    ...
    function VLtransfer(i, j) {
        ...
        */
        ...
        function calCurrProtection(S){
            ...
            if (nD >= S.vax_Tres) {
                vSector = 4;
                return (S.vax_LevelR);    // residual value
            };
            if (nD >= S.vax_Teff1 && nD < S.vax_Tdec){
                vSector = 2;
                return (S.vax_LevelP1);
            }
            if (nD < S.vax_Teff1){
                let rate1;
                rate1 = S.vax_LevelP1/S.vax_Teff1;
                vSector = 1;
                return ( rate1 * nD );
            }
            if (nD >= S.vax_Tdec && nD<S.vax_Tres){
              let rate2, loss, days, netloss, newlevel;
              loss = S.vax_LevelP1 - S.vax_LevelR;
              days = S.vax_Tres - S.vax_Tdec;
              rate2 = loss/days;
              netloss = rate2* (nD-S.vax_Tdec);
              newlevel = S.vax_LevelP1 - netloss;
              vSector = 3;
              return newlevel;
            }
        }

        function calcInfProb(S){
            if (S.vaxType == -1){
                return 1;             // not vaccinated but infective so prob = 1
            }
            // we have choice of random, or fixed, or function of the protection level of this agent
            // for now we use a randon number
            return (Math.random());
        }
        ...
        prProtect = calCurrProtection(P[Vsmall]);
        infProb   = calcInfProb(P[Vbig]);
        VaxProb = Math.random();

        if (VaxProb < prProtect) {
          P[Vsmall].touchCt++;
          console_log("Vax prevented "+Vsmall+" ageGp"+P[Vsmall].ageGp+" by "+Vbig+" ageGp"+P[Vbig].ageGp+" gen"+gen+" U"+wU+" Protection Level "+prProtect+" Prob "+VaxProb+" Sector"+vSector);
          return
        };

        TransProb = Math.random();
        if (TransProb > infProb) {
          console_log("No transmit "+Vbig+ " ageGp"+P[Vbig].ageGp+" on "+Vsmall+" ageGp"+P[Vsmall].ageGp+" gen"+gen+" "+wU+" Transmit Level "+infProb+" Prob "+TransProb);
          return
        };
...
//        };
    ...
    function  TimesUp() {
        gen++;
        if (gen % 24 == 0) {console_log("another day...: "+gen/24+" "+M.GreenCt)};
        let i;
...
function HALT(){
    ...
    function wrapUpBumps(){
        ...
        for (j=0;j<1001;j++){
            if (bin[j]!=0){
                console_log("Resistance bin"+j+": "+bin[j]);
            }
        }
    }
